...
import { RoleCommissionDetailDialog } from "./components/salesperson-commission-detail-dialog"
import { RoleCommissionEditDialog } from "./components/salesperson-commission-edit-dialog"
import { RoleCommissionAddDialog } from "./components/salesperson-commission-add-dialog"
...
export default function RoleCommissionPage() {
  ...
  const loadRoleCommissions = async (page: number = currentPage, limit: number = pageSize) => {
    try {
      setLoading(true)
      const response = await getRoleCommissionsPaginated({ page, limit, type: 'sales-person' })
      setRoleCommissions(response.data?.data || [])
      setTotalCount(response.data?.pagination?.total || 0)
      setError(null)
    } catch (err: unknown) {
      console.error("Error loading salesperson commissions:", err)
      const errorMessage = err instanceof Error ? err.message : "Failed to load salesperson commissions"
  ...
  const handleRowClick = (roleCommission: RoleCommission) => {
    if (roleCommission.salesPersonId?._id) {
      setSelectedSalesPersonId(roleCommission.salesPersonId._id)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Salesperson commission deleted successfully",
        variant: "success"
      })
    } catch (error: unknown) {
      console.error("Error deleting salesperson commission:", error)
      toast({
  ...
  const handleEditSuccess = () => {
    // Reload salesperson commissions after successful edit
    loadRoleCommissions()
  }

  const handleAddSuccess = () => {
    // Reload salesperson commissions after successful add
    loadRoleCommissions()
  }
  ...
  return (
    <>
      ...
      <Dialog open={!!deletingRoleCommission} onOpenChange={(open) => !open && setDeletingRoleCommission(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Salesperson Commission</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete the salesperson commission for &ldquo;{deletingRoleCommission?.salesPersonId?.name}&rdquo;?
