...
import { getRoleCommissionBySalesPersonId, updateRoleCommissionById } from "@/services/role-commission"
        ...
        if (initialRoleCommission && open) {
            const fetchRoleCommission = async () => {
                try {
                    ...
                    // Use salesperson ID for GET request
                    const data = await getRoleCommissionBySalesPersonId(initialRoleCommission.salesPersonId?._id || "")
        ...
        <Sheet open={open} onOpenChange={handleClose}>
            <SheetContent side="right"
                ...
                <SheetHeader className="bg-zinc-100/70 dark:bg-background/70 backdrop-blur-md border-b border-border/40">
                    <SheetTitle className="text-xl font-semibold py-0">Edit Salesperson Commission</SheetTitle>
                </SheetHeader>
                    ...
                    <div className="py-4 px-6">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <Input
                                    id="salespersonName"
                                    value={roleCommission.salesPersonId?.name || "N/A"}
                                    disabled
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="salespersonEmail">Salesperson Email</Label>
                                <Input
                                    id="salespersonEmail"
                                    value={roleCommission.salesPersonId?.email || "N/A"}
                                    disabled
                                    className="bg-muted/50"
                                />
                            </div>
