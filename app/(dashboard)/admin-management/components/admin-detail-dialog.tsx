            ...
            <SheetContent side="right"
                ...
                {admin && !loading && !error && (
                    <div className="space-y-8 py-4 px-6">
                        ...
                        {(admin.aadharCardNo || admin.panCardNo || admin.aadharCardImage?.length || admin.panCardImage?.length) && (
                            <section className="rounded-xl border border-border/50 dark:border-border/60 bg-card/30 dark:bg-card/50 p-6 space-y-6 shadow-sm">
                                <h2 className="text-lg font-medium text-foreground/80 tracking-tight">
                                    Identity Information
                                </h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    {/* Aadhaar Card Images */}
                                    {admin.aadharCardImage && admin.aadharCardImage.length > 0 && (
                                        <div className="md:col-span-2">
                                            <Label className="mb-2">Aadhaar Card Images</Label>
                                            <div className="flex flex-wrap gap-4 mt-2">
                                                {admin.aadharCardImage.map((image, index) => (
                                                    <div key={index} className="relative">
                                                        <img
                                                            src={image}
                                                            alt={`Aadhaar ${index === 0 ? 'Front' : 'Back'}`}
                                                            className="w-40 h-24 object-cover rounded border cursor-pointer hover:opacity-80 transition-opacity"
                                    ...
                                    {/* PAN Card Images */}
                                    {admin.panCardImage && admin.panCardImage.length > 0 && (
                                        <div className="md:col-span-2">
                                            <Label className="mb-2">PAN Card Images</Label>
                                            <div className="flex flex-wrap gap-4 mt-2">
                                                {admin.panCardImage.map((image, index) => (
                                                    <div key={index} className="relative">
                                                        <img
                                                            src={image}
                                                            alt={`PAN ${index === 0 ? 'Front' : 'Back'}`}
                                                            className="w-40 h-24 object-cover rounded border cursor-pointer hover:opacity-80 transition-opacity"
                                                            onClick={() => window.open(image, '_blank')}
                                                        />
                                                        <p className="text-xs text-center mt-1 text-muted-foreground">
