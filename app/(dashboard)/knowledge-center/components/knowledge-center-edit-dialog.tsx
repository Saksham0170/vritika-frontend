...
import { updateKnowledgeCenter } from "@/services/knowledge-center"
import { KnowledgeCenterEntry, UpdateKnowledgeCenterRequest } from "@/types/knowledge-center"
import { UPLOAD_ENDPOINTS } from "@/services/upload"
import { useToast } from "@/hooks/use-toast"

interface KnowledgeCenterEditDialogProps {
    open: boolean
    onClose: () => void
    onSuccess?: () => void
    entry: KnowledgeCenterEntry | null
}

export function KnowledgeCenterEditDialog({ open, onClose, onSuccess, entry }: KnowledgeCenterEditDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const requestData: UpdateKnowledgeCenterRequest = {
                type: formData.type,
                url: formData.url,
                status: formData.status,
            }

            await updateKnowledgeCenter(entry._id, requestData)

            toast({
                title: "Success",
                description: "Knowledge center entry updated successfully",
            })
            ...
            toast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to update knowledge center entry",
                variant: "destructive",
            })
        } finally {
            setSaving(false)
        ...
        <Sheet open={open} onOpenChange={handleClose}>
            <SheetContent side="right" className="w-[80vw] sm:w-[70vw] lg:w-[50vw] xl:w-[40vw] max-w-2xl overflow-y-auto rounded-2xl border border-border/40 bg-background text-foreground shadow-lg">
                <SheetHeader className="bg-background/70 backdrop-blur-md border-b border-border/40">
                    <SheetTitle className="text-xl font-semibold py-0">Edit Knowledge Center Entry</SheetTitle>
                </SheetHeader>
